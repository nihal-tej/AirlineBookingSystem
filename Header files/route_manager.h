...
public:
    ...
    vector<Flight> getFlightOptionsByTime(const string &src, const string &dst, TimeSlot slot, const string &date, const BookingSystem &system)
    {
        ...
        for (const auto &x : system.getFlights())
        {
            ...
            if (x.second.getdate() == date && x.second.getTime() >= low && x.second.getTime() <= high)
            {
                if (normalize(x.second.getSource()) == normalized_src &&
                    normalize(x.second.getDestination()) == normalized_dst)
                {
